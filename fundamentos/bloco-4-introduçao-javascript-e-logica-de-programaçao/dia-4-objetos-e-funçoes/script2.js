...
function repeatNumber (){
    ...
    let repetido1 = numbers[0];
    let repetido2 = numbers[1]
    let result;
    for(let index = 0; index < numbers.length; index += 1){
        if(numbers[index] === repetido1 && numbers[index] === repetido2){
...
function verificaFimPalavra (word, ending){
    for(let index = 0; index <= word; index += 1){
        if(ending.length-word.length === word){
            return true
        } 
        return false
        }
